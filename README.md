...
## 🚀 最简单：让 AI 帮你装（零代码基础，推荐）

如果你用的是能在自己电脑上帮你干活的 AI（如 **Claude Code、Codex、OpenClaw**），不用自己找文件夹，直接把这句话发给它：

> 请把这个技能安装到我电脑上：`https://github.com/pamelaaaaa1218/chinese-quote-normalizer`。装好后，以后我让你把引号转成中文时就用它。

AI 会自己下载文件、放到正确位置。**装这一次就行**，之后你只要说一句「用这个技能把这篇的引号转成中文」，它就会自动调用里面的脚本——又快又准，连代码块、网址、英文缩写里的引号都不会误伤，全程你不用碰任何代码。

> 只想**临时转一次**、或用的是普通网页版 AI？把文件（或链接）发给它，说「照这个技能把这段文字的引号转成中文」即可。注意：这种方式只对当前这次对话有效，且是 AI「照规则手动转」，偶尔可能漏一两个，重要文档建议按上面的方式正式安装、让它跑脚本。
